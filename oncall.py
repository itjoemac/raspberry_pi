...
# OnCall is a RPi visual indicator of your status while WFH
# using Python and a GPIO breadboard
# By J. McMahon, 2021
...
while choice != 'Q':
    ...
    if choice == 'G':
        while lightoff != 'S':
            lightg_init()
            GPIO.output(ledPinG, GPIO.HIGH)
            lightoff = input("You're available, press 'S' to stop.")
        ...
        while lightoff != 'S':
            lightr_init()
            GPIO.output(ledPinR, GPIO.HIGH)
            lightoff = input("You're on a conference call, press 'S' to stop.")
            lightoff = lightoff.upper()
            GPIO.output(ledPinR, GPIO.LOW) 
        continue
    elif choice == 'Y':
        while lightoff != 'S':
            lighty_init()
            GPIO.output(ledPinY, GPIO.HIGH)
            lightoff = input("You have headphones on, press 'S' to stop.")
            lightoff = lightoff.upper()
            GPIO.output(ledPinY, GPIO.LOW)  
        continue
    else:
